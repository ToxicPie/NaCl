...
struct linked_lists {
  ...
  explicit linked_lists(int L = 0, int N = 0) {
    assign(L, N);
  }

  int rep(int l) const {
    return l + N;
  } // "representative" of list l
  ...
  void clear() {
    iota(begin(next) + N, end(next),
         N); // sets next[rep(l)] = rep(l)
    iota(begin(prev) + N, end(prev),
         N); // sets prev[rep(l)] = rep(l)
  }
...
// Iterate through elements (call them i) of the list #l in
// "lists"
#define FOR_EACH_IN_LINKED_LIST(i, l, lists)               \
  for (int z##i = l, i = lists.head(z##i);                 \
       i != lists.rep(z##i); i = lists.next[i])

/**
 * Network simplex for minimum cost circulation with fixed
 * supply/demand at nodes. Supports negative costs, negative
 * cost cycles, self-loops and multiple edges fine.
 *
 * Flow should be large enough to hold sum of supplies and
 * edge flows Cost should be large enough to hold sum of
 * absolute costs * 2V (usually >=64 bits)
 *
...
struct network_simplex {
  ...
  void set_supply(int u, Flow supply) {
    node[u].supply = supply;
  }
  void set_demand(int u, Flow demand) {
    node[u].supply = -demand;
  }
  ...
  void verify_spanning_tree() const {
    for (int e = 0; e < E; e++) {
      assert(0 <= edge[e].flow &&
             edge[e].flow <= edge[e].cap);
      assert(edge[e].flow == 0 || reduced_cost(e) <= 0);
      assert(edge[e].flow == edge[e].cap ||
             reduced_cost(e) >= 0);
    }
  }

  bool mincost_circulation() {
    static constexpr bool INFEASIBLE = false,
                          OPTIMAL = true;

    // Check trivialities: positive cap[e] and sum of
    // supplies is 0
    Flow sum_supply = 0;
    for (int u = 0; u < V; u++) {
      sum_supply += node[u].supply;
    }
    if (sum_supply != 0) { return INFEASIBLE; }
    for (int e = 0; e < E; e++) {
      if (edge[e].cap < 0) { return INFEASIBLE; }
    }

    // Compute inf_cost as sum of all costs + 1, and reset
    // the flow network
    Cost inf_cost = 1;
    for (int e = 0; e < E; e++) {
      edge[e].flow = 0;
    ...
    // Add V artificial edges with infinite cost and initial
    // supply for feasible flow
    int root = V;
    node[root] = {-1, -1, 0, 0};
    ...
    for (int u = 0, e = E; u < V; u++, e++) {
      ...
      if (supply >= 0) {
        node[u].pi = -inf_cost;
        edge[e] = {
        {u, root}, supply, inf_cost, supply, STATE_TREE};
      } else {
        node[u].pi = inf_cost;
        edge[e] = {
        {root, u}, -supply, inf_cost, -supply, STATE_TREE};
      }
    }

    // We want to, hopefully, find a pivot edge in
    // O(sqrt(E)). This can be tuned.
    block_size =
    max(int(ceil(sqrt(E + V))), min(10, V + 1));
    next_arc = 0;
    ...
    // If there is >0 flow through an artificial edge, the
...
private:
  ...
  int select_pivot_edge() {
    ...
    for (int &e = next_arc; seen_edges-- > 0;
         e = e + 1 == E + V ? 0 : e + 1) {
  ...
  void pivot(int e_in) {
    ...
    // Go up from u_in to lca, break ties by prefering lower
    // vertices
    for (int u = u_in; u != lca && flow_delta > 0;
         u = node[u].parent) {
      int e = node[u].pred;
    ...
    // Go up from v_in to lca, break ties by prefering
    // higher vertices
    ...
    if (flow_delta > 0) {
      ...
      for (int u = edge[e_in].node[0]; u != lca;
           u = node[u].parent) {
        int e = node[u].pred;
        edge[e].flow +=
        u == edge[e].node[0] ? -delta : delta;
      }
      for (int u = edge[e_in].node[1]; u != lca;
           u = node[u].parent) {
        int e = node[u].pred;
        edge[e].flow +=
        u == edge[e].node[0] ? delta : -delta;
    ...
    // Return now if we didn't change the spanning tree. The
    ...
    // Basis exchange: Replace out_arc with e_in in the
    // spanning tree
    ...
    // Evert: Walk up from u_in to u_out, and fix
    ...
    // Fix potentials: Visit the subtree of u_in (pi_delta
    // is not 0).
